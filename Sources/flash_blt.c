...
#define FLASH_WRITE_BLOCK_SIZE          (0x200)
/** \brief Total numbers of sectors in array flashLayout[]. */
#define FLASH_TOTAL_SECTORS             (IFsh1_AREA_0_SIZE/IFsh1_AREA_SECTOR_SIZE)
...
#define BOOT_FLASH_VECTOR_TABLE_CS_OFFSET (0x1F2)
#endif
/** \brief Total size of the vector table, excluding the bootloader specific checksum. */
#define FLASH_VECTOR_TABLE_SIZE         (0x1F0)
#define FLASH_VECTAB_ADDR 				(0xFFFE10UL)
/** \brief Start address of the bootloader programmable flash. */
#define FLASH_START_ADDRESS             (IFsh1_AREA_0_START)
/** \brief End address of the bootloader programmable flash. */
#define FLASH_END_ADDRESS               (IFsh1_AREA_0_END)
/** \brief Size of a flash page on the HCS12. */
#define FLASH_PAGE_SIZE                 (IFsh1_AREA_SECTOR_SIZE)      /* flash page size in bytes      */
/** \brief Physical start address of the HCS12 page window. */
#define FLASH_PAGE_OFFSET               (0x0)      /* physical start addr. of pages */
/** \brief Base address of the flash related control registers. */
#define FLASH_REGS_BASE_ADDRESS         (0x0380)
...
#define FLASH_BOOTLOADER_SIZE			(0x1600UL)
...
blt_bool FlashWrite(blt_addr addr, blt_int32u len, blt_int8u *data)
{
  ...
  last_block_base_addr = FLASH_END_ADDRESS-FLASH_ERASE_BLOCK_SIZE+1;
...
blt_bool FlashErase(blt_addr addr, blt_int32u len)
{
  ...
  for (block_cnt=0; block_cnt<nr_of_erase_blocks; block_cnt++)
  {
    ...
    /* erase the block */
    IFsh1_EraseSector(erase_base_addr);
...
blt_bool FlashWriteChecksum(void)
{
  ...
  bootBlockInfo.data[2] = bootBlockInfo.data[508];
  bootBlockInfo.data[3] = bootBlockInfo.data[509];
  bootBlockInfo.data[4] = bootBlockInfo.data[510];
  bootBlockInfo.data[5] = bootBlockInfo.data[511];

  bootBlockInfo.data[508] = 0xff;
  bootBlockInfo.data[509] = 0xfe;
  bootBlockInfo.data[510] = 0x00;
  bootBlockInfo.data[511] = 0x03;
  ...
  /* write the checksum */
  checksum_address = FLASH_VECTAB_ADDR-0x10;
...
blt_bool FlashVerifyChecksum(void)
{
  ...
  checksum_addr_lin = (FLASH_VECTAB_ADDR-0x10);
  /* get linear address of the vector table start */
  vector_table_addr_lin = (FLASH_VECTAB_ADDR);
...
static blt_bool FlashInitBlock(tFlashBlockInfo *block, blt_addr address)
{
  ...
  IFsh1_GetBlockFlash(address, (blt_int8u*)&block->data, FLASH_WRITE_BLOCK_SIZE);
...
static blt_bool FlashWriteBlock(tFlashBlockInfo *block)
{
  ...
  if(IFsh1_SetBlockFlash((byte *)&block->data, block->base_addr, (FLASH_WRITE_BLOCK_SIZE/sizeof(blt_int8u))) != ERR_OK) {
	  result = BLT_FALSE;
  }
...
static blt_int8u FlashGetLinearAddrByte(blt_addr addr)
{
  ...
  IFsh1_GetByteFlash(addr, &result);
